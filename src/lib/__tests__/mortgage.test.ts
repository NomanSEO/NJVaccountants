...
describe('calculateMortgage', () => {
  ...
  it('keeps the principal outstanding for interest-only loans', () => {
    const result = calculateMortgage({ ...baseInput, loanType: 'interestOnly' })
  ...
  it('recalculates an adjustable-rate payment at its adjustment month', () => {
    ...
    expect(result.schedule[60].payment).toBeGreaterThan(result.schedule[59].payment)
  ...
  it('includes all annual escrow items in annual housing cost without treating them as interest', () => {
    ...
    expect(result.yearlyHousingCost).toBeCloseTo(result.monthlyHousingCost * 12, 8)
    expect(result.totalInterest).toBeCloseTo(370_682.20, 2)
  })
})
