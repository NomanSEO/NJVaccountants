...
const monthlyPayment = (principal: number, annualRate: number, remainingMonths: number) => {
...
export function calculateMortgage(input: MortgageInput): MortgageResult {
  ...
  const initialPayment = input.loanType === 'interestOnly'
  ...
  for (let month = 1; month <= termMonths; month += 1) {
    if (input.loanType === 'adjustable' && month === input.adjustmentMonth) {
      ...
      payment = monthlyPayment(balance, input.adjustedAnnualRate, termMonths - month + 1)
    }

    const interest = balance * monthlyRate
    const principal = input.loanType === 'interestOnly' ? 0 : Math.min(balance, payment - interest)
