...
const paymentForBalance = (
  ...
  return Math.max(
    0,
    ((balance * growth - residual) * monthlyRate) / (growth - 1),
  );
};
...
export function calculateLoanSchedule(input: LoanScheduleInput): LoanResult {
  const financedAmount =
    Math.max(0, input.principal) + Math.max(0, input.financedFees);
  const termMonths = Math.max(0, Math.floor(input.termMonths));
  const deferredMonths = Math.min(
    termMonths,
    Math.max(0, Math.floor(input.deferredMonths)),
  );
  ...
  for (let period = deferredMonths + 1; period <= termMonths; period += 1) {
    ...
    const principal =
      input.loanType === "interestOnly"
        ? 0
        : Math.min(
            Math.max(0, balance - requestedBalloon),
            Math.max(0, periodicPayment - interest),
          );
    const payment =
      input.loanType === "interestOnly" ? interest : principal + interest;
