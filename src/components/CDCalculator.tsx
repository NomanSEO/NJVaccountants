...
export default function CDCalculator() {
  ...
  const result = useMemo(() => {
    ...
    const balance = valid
      ? principal * Math.pow(1 + rate / compounds, periods)
      : 0;
    const rows = Array.from({ length: Math.ceil(term) }, (_, index) => {
      const completed = Math.min(index + 1, term);
      const value =
        principal * Math.pow(1 + rate / compounds, completed * compounds);
    ...
    <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]">
      ...
      >
        ...
        <p className="text-slate mt-2 text-sm leading-relaxed">
          Estimate the value of one fixed deposit at maturity.
        </p>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <Field label="Currency" id="cd-currency">
            <select
              id="cd-currency"
              value={currency}
          ...
          <Field label="Compounding" id="cd-frequency">
            <select
              id="cd-frequency"
              value={frequency}
              onChange={(e) =>
          ...
          <NumberField
            ...
            onChange={setDeposit}
            error={
              principal <= 0 ? "Enter a deposit greater than zero." : undefined
            }
          />
          <NumberField
            label="APY (%)"
            ...
            step="0.01"
            error={
              rate < 0 || rate > 1 ? "Enter a rate from 0% to 100%." : undefined
            }
          />
          <NumberField
            label="Term (years)"
      ...
      >
        ...
        {valid ? (
          <div className="mt-7 grid gap-4 sm:grid-cols-2">
            ...
            />
            <Stat label="APY" value={`${number(apy).toFixed(2)}%`} />
          </div>
        ) : (
        ...
        <p className="mt-5 text-xs leading-relaxed text-white/55">
          This illustration assumes the APY remains fixed and interest stays in
          the account.
        </p>
      </section>
      {valid && (
        <section className="border-border space-y-8 rounded-sm border bg-white p-4 sm:p-6 md:p-8 lg:col-span-2">
          <ResultBarChart
            title="Deposit and interest"
            items={[
              {
                name: "Initial deposit",
                value: principal,
                formattedValue: formatCurrency(principal, currency),
              },
              {
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="text-slate mt-1 text-sm">
                Projected balance through the maturity date.
              </p>
            </div>
            <CalculatorActions
              filename="cd-projection.csv"
              shareTitle="CD calculator estimate"
              rows={[
          ...
          <Table rows={visible} currency={currency} />
...
function Table({
  ...
  rows: { year: number; balance: number; interest: number }[];
  ...
  return (
    <div className="overflow-x-auto" data-calculator-table>
      ...
      <table className="w-full min-w-130 text-left text-sm">
        <caption className="sr-only">CD yearly balance projection</caption>
        <thead className="border-border text-slate border-b text-xs tracking-[.08em] uppercase">
          <tr>
            {["Year", "Balance", "Interest earned"].map((label) => (
