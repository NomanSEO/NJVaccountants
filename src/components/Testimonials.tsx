...
export default async function Testimonials() {
  ...
  return (
    <section id="testimonials" className="py-24 bg-navy" aria-label="Client testimonials">
      <div className="max-w-site mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {items.map(t => (
            <article key={t._id} className="bg-navy-light rounded-sm border border-border/20 p-9 relative hover:shadow-[0_12px_40px_rgba(11,31,58,0.07)] transition-shadow">
              ...
              <p className="text-[0.9375rem] text-cream leading-[1.75] mb-7 italic">{t.quote}</p>
              <div className="flex items-center gap-3.5 border-t border-border/20 pt-5">
                ...
                <div>
                  <div className="font-semibold text-[0.9rem] text-cream">{t.authorName}</div>
                  <div className="text-[0.8rem] text-cream/70">{t.authorRole}</div>
                </div>
              </div>
            </article>
          ))}
