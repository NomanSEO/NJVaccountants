      ...
      <div className="max-w-site mx-auto px-6">
        <header className="mb-14">
          ...
          <h2 className="font-display text-[clamp(2rem,3.5vw,2.75rem)] font-bold text-navy leading-tight mb-5">
            Results That <em className="not-italic text-gold">Speak</em> for
            Themselves
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-7">
          {cases.map((c) => (
            <article
              key={c._id}
              className="rounded-sm overflow-hidden border border-border hover:shadow-[0_16px_48px_rgba(11,31,58,0.1)] hover:-translate-y-1 transition-all duration-300"
            >
              ...
              <div className="p-7">
                <div className="text-[0.7rem] tracking-[0.1em] uppercase text-slate-light mb-2">
                  The Challenge
                </div>
                <p className="text-[0.9rem] text-text leading-[1.6] mb-6">
                  {c.challenge}
                </p>
                <div className="grid grid-cols-2 gap-4 pt-5 border-t border-border">
                  {c.results.map((r) => (
                    <div key={r.label}>
                      <div className="font-display text-[1.625rem] font-bold text-navy">
                        {r.num}
                      </div>
                      <div className="text-[0.75rem] text-slate-light mt-0.5">
                        {r.label}
                      </div>
                    </div>
                  ))}
        ...
        <div className="mt-12 text-center">
          <a
            href="#contact"
            className="inline-block px-8 py-4 bg-gold text-navy font-semibold rounded hover:bg-gold-light transition-colors"
          >
